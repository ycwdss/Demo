...
define(['jquery','fhlib/cookie','fhlib/totop',
    ...
    var markerArr1 = [{
        title: "凤凰筑巢总部",
        content: "金水区郑汴路与玉凤路凤凰城南城东南区6楼",
        point: "113.719501|34.748911",
        isOpen: 1,
...
//创建和初始化地图函数：
    function initMap(markerArr1) {
        createMap();//创建地图
        setMapEvent();//设置地图事件
        addMapControl();//向地图添加控件
        addMarker(markerArr1);//向地图中添加marker
    }
    //创建和初始化地图函数：
    function initMap2(markerArr2) {
        createMap2();//创建地图
        setMapEvent();//设置地图事件
        addMapControl();//向地图添加控件
        addMarker(markerArr2);//向地图中添加marker
    }
...
//创建地图函数：
    ...
    function createMap2() {
        var map = new BMap.Map("map2");//在百度地图容器中创建一个地图
        var point = new BMap.Point(113.718957, 34.749541);//定义一个中心点坐标
        map.centerAndZoom(point, 18);//设定地图的中心点和坐标并将地图显示在地图容器中
        window.map = map;//将map变量存储在全局
    }
...
//创建一个Icon
    ...
    initMap(markerArr1);//创建和初始化地图
    initMap2(markerArr2);//创建和初始化地图
});